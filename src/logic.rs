...
pub fn find_triangular_opportunities(
    ...
    // Build fast lookup map: (base, quote) -> price
    ...
    for p in &pairs {
        symbols.insert(p.base.clone());
        symbols.insert(p.quote.clone());
    }
    let symbols: Vec<String> = symbols.into_iter().collect();

    let mut out: Vec<TriangularResult> = Vec::new();

    // Instead of brute force O(n^3), we walk only connected triples
    for a in &symbols {
        for b in &symbols {
            if a == b { continue; }
            // require A/B exists
            if !map.contains_key(&(a.clone(), b.clone())) { continue; }

            for c in &symbols {
                if c == a || c == b { continue; }

                // require B/C and C/A exist
    ...
    // Sort opportunities by highest profit_after
